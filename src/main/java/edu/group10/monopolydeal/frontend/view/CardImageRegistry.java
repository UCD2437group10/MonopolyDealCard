...
/**
 * A utility registry responsible for resolving card names to their corresponding bundled image resources.
 * It handles name aliasing, builds standardized JavaFX {@link ImageView} instances, and maintains an
 * in-memory cache of decoded {@link Image} objects to optimize memory usage and rendering performance.
 */
public final class CardImageRegistry {

    /** The base classpath directory containing all packaged card artwork. */
    private static final String CARD_IMG_BASE = "/images/cards/";

    /** A mapping from canonical card names to their base image filenames. */
    ...
    /** * A mapping from runtime aliases to canonical card names.
     * This provides flexibility for matching alternate string representations generated by the backend.
     */
    ...
    /** An in-memory cache of previously decoded images to prevent redundant file I/O operations. */
    ...
    /**
     * Resolves the resource path for a given card and constructs a standardized {@link ImageView}.
     * The resulting view is pre-configured with fixed dimensions (160x228),
     * ratio preservation, and smooth scaling.
     *
     * @param card the target {@link Card} to generate a view for.
     * @return a configured {@link ImageView} containing the card's artwork, or {@code null} if the image cannot be resolved or loaded.
     */
    ...
    /**
     * Retrieves an image from the cache, or loads it from the classpath if not already cached.
     *
     * @param resourcePath the absolute classpath path to the image resource.
     * @return the loaded {@link Image}, or {@code null} if the path is invalid or an I/O error occurs.
     */
    ...
    /**
     * Retrieves the resource path for the standard card-back image.
     *
     * @return the classpath resource string for the card back.
     */
    ...
    /**
     * Resolves a card's name (or runtime alias) to its actual physical resource path.
     * <p>
     * Note: This method automatically appends " new.jpg" to the resolved base filename
     * based on the current asset naming convention for updated artwork.
     *
     * @param cardName the raw name or alias of the card.
     * @return the resolved classpath resource string, or {@code null} if the resource cannot be mapped or found.
     */
